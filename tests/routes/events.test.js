...
// Jest test for delete event/:id
describe("DELETE event by ID", () => {
 const endpoint = "/api/events";
 it("should return a 200 status code if the event is deleted successfully", async () => {
   const res = await request(app)
     .post(endpoint)
     .send({
       location_id: 1,
   ...
   const res2 = await request(app)
     .delete(`${endpoint}/${res.body.event_id}`)
     .set({
       Authorization: admin,
     });
 ...
 it("should return a 400 status code if no token is provided", async () => {
   const res = await request(app).delete(`${endpoint}/1000`);
   expect(res.statusCode).toBe(400);
   expect(res.body.error).toBe("Token missing from Authorization header");
 });
});
